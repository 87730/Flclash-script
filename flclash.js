/**
 * mihomo / Clash 极简自适应覆写脚本 (Flclash 安卓定制优化版 - 修正版)
 *
 * 核心设计：
 * 1. 严格一字不改保留原作者完整 DNS 处理逻辑体系（从 commonDnsList 到 buildDnsAndHostsConfig 全套算法完整保留，
 *    自动继承原机场 hosts 与私有 DNS，确保专线/中转机场精准匹配 BGP 入口，绝对不被分配透传 IP）。
 * 2. 彻底解决原版手机端痛点：剔除所有后台自动测速组与冗余地区组，界面仅留【默认代理】、【漏网之鱼】、【直连】。
 * 3. 严格遵循三段式分流（内网直连 + 国内直连 + 国外走代理）。
 * 4. 剔除桌面端系统级配置（tun/mixed-port/external-controller），由 Flclash 原生安全接管。
 */

// 排除非节点的垃圾广告正则（与原版保持一致）
const excludeFilter =
...
// 屏蔽国外QUIC（严格采用原版逻辑与 no-resolve，杜绝解析触发与泄露）
...
/**
 * 生成 DNS 和 hosts 相关配置（完全保持原作者 100% 原始逻辑）
 */
...
function main(config) {
  ...
  // 3. 构建极简策略组（仅 3 个组，彻底杜绝手机后台测速耗电）
  // 命名采用“默认代理”，与 foreignDNS 的引用 (#默认代理) 100% 对应无缝契合
  const proxyGroups = [
    {
      ...selectBaseOption,
      name: '默认代理',
  ...
  // 6. 移除桌面端强行绑定的参数，由 Android Flclash 自身管理 VPNService
