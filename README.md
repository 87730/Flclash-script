...
- **大文件满速直连防偷跑**：独立分离苹果服务（`apple-cn.mrs`）与微软服务（`microsoft@cn.mrs`）走内核原生 `DIRECT` 直连出站，保障 App Store 软件下载、iOS 固件更新与 Windows Update 累积补丁满速狂飙，1KB 都不偷跑机场海外流量。
...
- **严密防 DNS 泄露**：启用 Fake-IP 地址池（`198.18.0.1/15` 与 `2001:2::1/48` 跨平台高兼容防冲突网段），国外域名通过加密通道进行远端代理解析，国内域名走直连解析，开启 `direct-nameserver-follow-policy` 优先遵从纯净直连解析策略。
...
- **性能与保活优化**：测速接口统一为 Google 原生轻量探针（`gstatic.com/generate_204`）并增加 `204` 状态码强校验（杜绝节点欠费跳转假通畅），关闭后台无意义进程扫描，加入节点重名自动编号容错、双栈并发择优、自动补全 Chrome 浏览器指纹、doh.pub 物理 IP 防死锁及科学 TCP 双保活调优。
