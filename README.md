...
- **极致纯净·零冗余**：彻底剔除所有花里胡哨的细分地区分组与繁杂的应用分流组，手机主界面仅保留 3 个核心卡片，告别滑动地狱。
...
- **内置纯净自动选择**：在【默认代理】节点列表顶部内置无 Emoji 的纯文字 `自动选择` 选项，采用 `lazy: true` 惰性测速，不勾选时后台完全休眠零发包，兼顾省电与自动兜底。
- **FlClash 移动端专属优化**：
  - 测速地址统一为 0 字节轻量接口（`https://cp.cloudflare.com/generate_204`），零流量损耗；
