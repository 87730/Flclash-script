...
- **极致纯净·仅 2 卡片**：彻底剔除所有细分地区分组、应用分流组、自动测速组与冗余的兜底卡片。手机主界面仅保留 `默认代理` 与 `直连` 2 个核心卡片，达到终极极简。
- **纯粹闭环分流**：局域网直连 -> 屏蔽国外 QUIC（防卡顿）-> 国内域名与 IP 直连 -> 国外流量与末尾兜底全走 `默认代理`（国内走国内，国外走国外）。
...
```text
  默认代理 (包含订阅所有有效真实节点，纯手动选择，同时负责全量未知国外流量兜底)
  直连     (本地网络直接连接)
```
