...
- **单卡片极简结构**：主界面仅保留唯一的核心卡片 `默认代理`，去除多余策略卡片，告别滑动地狱。
- **白名单分流**：采用 MetaCubeX 官方纯大陆标准 `cn.mrs`，国内服务与局域网直接走内核原生 `DIRECT` 直连，国外流量通过 `默认代理` 转发。
...
- **移动端优化**：测速接口统一为轻量 0 字节探针，关闭后台无意义进程扫描，加入节点重名自动编号容错与连接保持。
